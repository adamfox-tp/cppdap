...
var (
	dapVersion       = flag.String("dap-version", "", "DAP version tag to use (defaults to master if not specified)")
	vendorExtensions = flag.String("vendor-extensions", "", "Path to vendor extensions JSON file")
)

func gitReferenceFromDAPVersion(version string) string {
	if version == "" {
		return "master"
	}
...
// buildProtocolURL constructs the URL for the debugProtocol.json file
func buildProtocolURL(version string) string {
	return fmt.Sprintf("https://raw.githubusercontent.com/microsoft/vscode-debugadapter-node/%s/debugProtocol.json", gitReferenceFromDAPVersion(version))
}

// buildPackageURL constructs the URL for the package.json file
func buildPackageURL(version string) string {
	return fmt.Sprintf("https://raw.githubusercontent.com/microsoft/vscode-debugadapter-node/%s/protocol/package.json", gitReferenceFromDAPVersion(version))
}
...
func run() error {
	version := *dapVersion
	packageURL := buildPackageURL(version)
	protocolURL := buildProtocolURL(version)
	...
	// Handle vendor extensions if specified
	if *vendorExtensions != "" {
		vendorProtocol := root{}
		if err := loadVendorExtensionsFile(*vendorExtensions, &vendorProtocol); err != nil {
			return fmt.Errorf("Failed to load vendor extensions file from '%v': %w", *vendorExtensions, err)
		}

		// Merge vendor definitions directly into the main protocol
		if protocol.Definitions == nil {
			protocol.Definitions = make(map[string]*definition)
		}
		
		// Add vendor definitions to the main protocol
		for name, def := range vendorProtocol.Definitions {
			protocol.Definitions[name] = def
		}
	}
...
// loadVendorExtensionsFile() loads the vendor extensions JSON file from the local file system.
func loadVendorExtensionsFile(filepath string, obj interface{}) error {
